...
export const filteredCountries = createSelector(selectAll, filters, fromCities.citiesByCountry, fromCities.selectEntities,
  (countries, filter, citiesByCountry, cities) => {
    ...
    const sortedList = sortCountries(filteredList, cityEntities, +filter.sortBy as CountrySortOption);

    return sortedList;
  });

export const filteredCountriesLength = createSelector(filteredCountries, countries => countries && countries.length);
...
export const sortCountries = (countries: Array<Country>, citiesByCountries, sortByOption: CountrySortOption) => {
  let sort;

  switch (sortByOption) {
    case CountrySortOption.PriceDown: {
      sort = (a: Country, b: Country) =>
        price(citiesByCountries[a.code]).price < price(citiesByCountries[b.code]).price ? -1 : 1;
      break;
    }
    ...
    case CountrySortOption.ElectorateDown: {
      sort = (a: Country, b: Country) =>
        electorate(citiesByCountries[a.code]) < electorate(citiesByCountries[b.code]) ? -1 : 1;
      break;
    }
    ...
    case CountrySortOption.NumberOfCitiesDown: {
      sort = (a: Country, b: Country) =>
        numberOfCities(citiesByCountries[a.code]) < numberOfCities(citiesByCountries[b.code]) ? -1 : 1;
      break;
    }
...
export const filteredCities = createSelector(fromCities.selectAll, fromCities.filters, selectEntities, (cities, filter, countries) => {
  const upperQuery = filter.query && filter.query.toUpperCase();
...
export const filteredCitiesLength = createSelector(filteredCities, cities => cities && cities.length);
