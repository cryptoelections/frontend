  ...
  template: `
    <accordion-group #group>
      <div accordion-heading class="row my-country-header">
        ...
        <ng-container class="country-header-wrap">
          <div class="country-name col-md-4 col-xs-12" [tooltip]="country.name" position="auto"><h2>{{ country.name
            }}</h2></div>
          <div class="progressbar col-md-4 col-xs-12">
            <progressbar max="100" [value]="percentage / 2">
              <i>{{ citiesOfCountry && citiesOfCountry.length }}
                {{ citiesOfCountry.length === 1 ? 'CAMPAIGN.CITY' : 'CAMPAIGN.CITIES' | translate}}</i></progressbar>
      ...
      <ng-container *ngFor="let city of citiesOfCountry; let l = last">
        <app-my-city [ngClass]="{'my-city-row-bottom': !l }" [city]="city"
